  ...
  return (
    ...
    >
      <div className="flex flex-col items-center w-full max-w-sm px-8">
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.875rem', marginBottom: '2rem', textAlign: 'center' }}>
        ...
        <div className="w-full flex flex-col gap-3.5" style={{ marginBottom: '2rem' }}>
          {POINTS.map((p, i) => (
            <div key={i} className="flex items-start gap-3">
              ...
              <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.85rem', lineHeight: '1.5' }}>{p.text}</span>
            </div>
          ))}
        </div>
        ...
        <button
          onClick={onAccept}
          className="w-full rounded-xl font-semibold py-3 transition-opacity hover:opacity-90 active:opacity-75"
          style={{ background: '#FF4D4D', color: 'white', fontSize: '0.95rem' }}
        >
          Accept &amp; Get Started
        </button>

        {/* Legal footer */}
        <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.75rem', marginTop: '1rem', textAlign: 'center', lineHeight: '1.5' }}>
          By continuing you agree to our{' '}
          <span style={{ color: 'rgba(255,255,255,0.45)', textDecoration: 'underline' }}>
            Terms of Service
          </span>
